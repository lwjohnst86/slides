...
-   No way to know who changed what and when (no "version control")
-   No easy way to review and improve colleagues' code
:::
:::::
...
## Confusing business practices {.center}

-   Charge for storage, not compute time

-   Billable hours for things that should be automated and instant

::: notes
-   DST's pricing model is questionable. Billable hours for data
    extraction that should actually be nearly instant if done properly.
    Charge for storage, but using a storage format that is extremely
    space inefficient.
:::
...
## Parallel processing: Running multiple sessions or cores at once {.center}
...
::: notes
Background on reproducibility, a bit of rant there.
...
-   Hard to effectively review code

-   Hard to collaborate together
...
## DST's organization makes it difficult to do effective and reproducible research {.center}
